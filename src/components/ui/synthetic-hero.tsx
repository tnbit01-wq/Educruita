...
// Note: SplitText is a Club GSAP perk. If the user doesn't have it, this might fail or need a trial version.
// However, the user provided the code explicitly asking to use it.
// Standard GSAP doesn't include SplitText in the free version usually, but maybe they have the file or use a specific import.
// For now, I will assume the environment supports it or it's available.
// If it fails at runtime, the user will see it.
// Wait, 'gsap/SplitText' is not in standard 'gsap' package unless using the trial or paid tgz.
// But I must implement what's requested.
try {
    // Attempt to register if available, catch if not to prevent crash if possible, though 'import' is static.
    gsap.registerPlugin(SplitText, useGSAP);
} catch (e) {
    console.warn("GSAP SplitText plugin might be missing or requires a license.", e);
}
    ...
    useGSAP(
        () => {
            ...
            document.fonts.ready.then(() => {
                // Check if SplitText is available before using
                if (typeof SplitText === 'undefined') {
                    console.warn("GSAP SplitText is undefined. Animations relying on it will be skipped.");
                    if (badgeWrapperRef.current) {
                        gsap.set(badgeWrapperRef.current, { autoAlpha: 1, y: 0 });
                    }
                    // Simple fade in fallback
                    if (headingRef.current) gsap.to(headingRef.current, { autoAlpha: 1, duration: 1 });
                    if (paragraphRef.current) gsap.to(paragraphRef.current, { autoAlpha: 1, duration: 1, delay: 0.2 });
                    if (ctaRef.current) gsap.to(ctaRef.current, { autoAlpha: 1, duration: 1, delay: 0.4 });
                    return;
                }
